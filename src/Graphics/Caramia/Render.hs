...
data DrawParams s = DrawParams
    {
      pipeline      :: Shader.Pipeline s
    ...
    , targetFramebuffer :: FBuf.Framebuffer s
    -- ^ Where do you want to render?
    , bindTextures  :: IM.IntMap (Texture s)
...
defaultDrawParams :: DrawParams s
...
data DrawCommand s = DrawCommand
    { primitiveType :: Primitive
    , primitivesVAO :: VAO.VAO s  -- ^ This is the VAO from which attributes
...
draw :: DrawCommand s -> DrawParams s -> Context s ()
draw cmd params = runDraws params (drawR cmd)

-- | Same as `draw` but in a `Draw` command stream.
drawR :: DrawCommand s -> Draw s ()
drawR (DrawCommand {..})
    | numIndices == 0 = return ()
    | otherwise = Draw $ do
    old_ebo <- boundEbo <$> get
    lift $ withResource (VAO.resource primitivesVAO) $ \(VAO.VAO_ vao_name) ->
...
data DrawState s = DrawState
    { boundPipeline :: !(Shader.Pipeline s)
    , boundEbo :: !GLuint
    , boundTextures :: !(IM.IntMap (Texture s))
    , boundBlending :: !BlendSpec
    , boundFramebuffer :: !(FBuf.Framebuffer s)
    , boundFragmentPassTests :: !FragmentPassTests
    , activeTexture :: !GLuint }

newtype Draw s a = Draw (StateT (DrawState s) (Context s) a)
...
runDraws :: DrawParams s    -- ^ Initial drawing parameters. These can be
                            --   changed in the `Draw` command stream.
         -> Draw s a        -- ^ Draw command stream.
         -> Context s a
...
withParams :: DrawParams s -> Context s a -> Context s a
withParams (DrawParams {..}) action =
    ...
    withPolygonOffset polygonOffset $ do
        ...
        liftIO $ allocaArray 4 $ \viewport_ptr -> unsafeResumeContext st $ do
            ...
            ox <- liftIO $ peekElemOff viewport_ptr 0
            oy <- liftIO $ peekElemOff viewport_ptr 1
            ow <- liftIO $ peekElemOff viewport_ptr 2
            oh <- liftIO $ peekElemOff viewport_ptr 3
            finally (glActiveTexture gl_TEXTURE0 *>
                    action)
                    (glActiveTexture old_active *>
                     glViewport ox oy ow oh)

withPolygonOffset :: (Float, Float) -> Context s a -> Context s a
...
setTextureBindings texes = do
        ...
        case IM.lookup index texes of
            ...
            Nothing -> setActiveTexture (safeFromIntegral index) *>
                let (bind_target, _) =
                 ...
                 in Draw $ lift $ glBindTexture bind_target 0
            -- A texture was bound previously, new bindings also bind some
            -- texture here.
            Just new_tex -> do
                old_name <-
                    Draw $ lift $ withResource (Texture.resource tex) $
                        \(Texture.Texture_ old_name) -> return old_name
                name <-
                    Draw $ lift $ withResource (Texture.resource new_tex) $
                ...
                when (name /= old_name) $ do
                    ...
                    let (bind_target, _) =
                        ...
                        in Draw $ lift $ glBindTexture bind_target name
    ...
    for_ (IM.assocs texes) $ \(index, tex) ->
        case IM.lookup index old_texes of
            ...
            Nothing -> do
                name <- Draw $ lift $ withResource (Texture.resource tex) $
                            \(Texture.Texture_ name) -> return name
                setActiveTexture (safeFromIntegral index)
                let (bind_target, _) =
                        Texture.getTopologyBindPoints $
...
setPipeline :: Shader.Pipeline s -> Draw s ()
setPipeline pl = Draw $ do
    old_pl <- boundPipeline <$> get
    when (old_pl /= pl) $ do
        lift $ withResource (Shader.resourcePL pl) $
...
withPipeline :: Shader.Pipeline s -> Context s a -> Context s a
